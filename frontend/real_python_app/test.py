...
class FlaskTestCase(unittest.TestCase):
	...
	# Ensure that login behaves correctly with credentials
	...
	# Ensure that login behaves correctly with wrong credentials
    def test_incorrect_login(self):
        tester = app.test_client(self)
        response = tester.post('/login',data=dict(username="wrong", password="wrong"), follow_redirects=True)
        self.assertIn(b'invalid credentials. pleade try again', response.data)
	...
	# Ensure that that main page requires login
